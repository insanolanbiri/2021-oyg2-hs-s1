...
def index():
    ...
    if request.cookies.get("tarih"):
        ...
        return render_template(
            "index.html", ad=deger, soyad=deger2, tarih=tarih, sayfa_adi="insanolanbiri"
        )
    else:
        res = make_response(
            render_template(
...
def liste():
    ...
    try:
        ...
        if file:
            file.close()
    return render_template(
        "liste.html", kisiler=kisiler, sayfa_adi="liste | insanolanbiri"
    )
...
def kayit():
    return render_template(
        "kayit_formu.html", hata="henüz hata yok", sayfa_adi="kayıt | insanolanbiri"
    )
...
def kaydet():
    ...
    if not (ad and soyad and program):
        return render_template(
            "kayit_formu.html", hata="bilgiler eksik", sayfa_adi="kayıt | insanolanbiri"
        )
    try:
        yol = os.path.join(os.getcwd(), "kayitlar.csv")
        ...
        return render_template(
            "kayit_formu.html",
            hata="veri kaydedilemedi",
            sayfa_adi="kayıt | insanolanbiri",
        )
    finally:
        if file:
            file.close()
...
def grafik():
    if request.method == "GET":
        return render_template(
            "grafik.html",
            resim="<p>henüz grafik yok</p>",
            sayfa_adi="grafik | insanolanbiri",
        )
    else:
        try:
            ...
            return render_template(
                "grafik.html",
                resim="<p>doğru düzgün gir</p>",
                sayfa_adi="grafik | insanolanbiri",
            )
        except:
            return render_template(
                "grafik.html",
                resim="<p>bir şey oldu</p>",
                sayfa_adi="grafik | insanolanbiri",
            )
        img = io.BytesIO()
        plt.savefig(img, format="png", bbox_inches="tight")
        img.seek(0)
        html_resim = '<img src="data:image/png;base64, {}">'.format(
            base64.b64encode(img.getvalue()).decode("utf-8")
        )
        return render_template(
            "grafik.html", resim=html_resim, sayfa_adi="grafik | insanolanbiri"
        )
